...
public class AuthServiceImpl implements AuthService {
    ...
    public UserResponse loginUser(UserRequest userRequest) {
        ...
        // Validate the user's password (method already handled)
        ...
        // Map the user entity to UserResponse and set the token
    ...
    public void changePassword(ChangePasswordRequest request) {
        ...
        User user = userRepository.findById(request.getUserId())
                .orElseThrow(() -> new GlobalExceptionHandler.ResourceNotFoundException("User not found with id: " + request.getUserId()));

        // Validate the current password using PasswordEncoder's matches() method
        if (!passwordEncoder.matches(request.getCurrentPassword(), user.getPassword())) {
            throw new GlobalExceptionHandler.InvalidCredentialsException("Current password is invalid");
        }
